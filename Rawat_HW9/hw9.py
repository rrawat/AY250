...
for dart_throws in [10,100,1000, 10000, 100000,1000000]:
	...
	simple_data= simple_method(dart_throws); simple_list.append(simple_data)
	IP_data= IPcluster_method(dart_throws); IP_list.append(IP_data)
	multi_data= multiprocessing_method(dart_throws); multiprocessing_list.append(multi_data)
	
	for data in [simple_data, IP_data, multi_data]:
		if data > time_limit:
			break
...
simple = np.array(simple_list)
ip = np.array(IP_list)
multi = np.array(multiprocessing_list)
...
ax.plot(darts,multi,'g', label = 'multi', lw=2,  marker='o', ms=5)
